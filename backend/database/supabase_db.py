...
def _get_headers(access_token: str | None = None):
...
async def save_analysis(
    ...
    try:
        ...
        detail = f" - {response.text}" if response is not None else ""
        logger.error(f"Failed to save analysis to Supabase: {exc}{detail}")
        return None

async def get_user_history(user_id: str, access_token: str | None = None) -> List[Dict]:
    headers = _get_headers(access_token)
    if not headers:
        return []
    ...
    try:
        ...
        response = getattr(exc, "response", None)
        detail = f" - {response.text}" if response is not None else ""
        logger.error(f"Failed to fetch history from Supabase: {exc}{detail}")
        return []
...
async def delete_analysis(
    ...
    try:
        ...
        response = getattr(exc, "response", None)
        detail = f" - {response.text}" if response is not None else ""
        logger.error(f"Failed to delete analysis {analysis_id}: {exc}{detail}")
        return False
