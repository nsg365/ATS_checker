...
async def analyze_resume(
    request: Request,
    resume: UploadFile = File(..., description='Resume file - PDF or DOCX, max 10 MB'),
    ...
    try:
        ...
        await save_analysis(user_id, filename, result, access_token)
    except Exception as exc:
        logger.warning(f'History save failed (non-blocking): {exc}')
        warnings.append('Your analysis could not be saved to your history.')
...
async def get_history(current_user: dict = Depends(get_current_user)):
    ...
    try:
        return await get_user_history(current_user['id'], current_user['access_token'])
    except Exception as exc:
...
async def generate_history_pdf(
    ...
    history = await get_user_history(current_user['id'], current_user['access_token'])
