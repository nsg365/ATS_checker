...
def display_detailed_feedback(analysis: Dict[str, Any]) -> None:
    all_issues = analysis.get("detailed_feedback") or []
    # High/Critical items are surfaced in full by the Critical Issues section,
    # so show only the remaining (Medium/Low) items here to avoid repeating them.
    issues = [
        i for i in all_issues
        if (i.get("severity_level") or "low").lower() not in ("high", "critical")
    ]
    if not issues:
        return  # nothing left once critical items are shown above

    st.markdown("### 🔍 Detailed Feedback")
    st.caption(f"{len(issues)} additional issue(s) - grouped by severity.")
